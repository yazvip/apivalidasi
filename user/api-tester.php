...
try {
    ...
    $stmt = $pdo->prepare("SELECT api_key, name FROM api_keys WHERE user_id = ? AND is_active = 1");
                            ...
                            <div class="mb-4">
                                ...
                                <select class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200" 
                                    ...
                                    <option value="<?= htmlspecialchars($key['api_key']) ?>">
                                        <?= htmlspecialchars($key['name']) ?> (<?= htmlspecialchars($key['api_key']) ?>)
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                                <p class="text-sm text-gray-500 mt-1">Select from your active API keys</p>
                            </div>
